...
- `stock_bid_ask_em`
  用途：查询单只股票盘口和买卖档位数据。
- `stock_index_zh_hist`
  用途：查询指数日线（如沪深300、中证500），symbol 如 000001/399001。
- `stock_financial_abstract`
  用途：查询单只股票财务摘要，symbol 必填。
- `stock_yjbb_em`
  用途：业绩快报，date 如 20241231。
- `stock_yjyg_em`
  用途：业绩预告，date 如 20241231。

### 期权

- `option_finance_board`
  用途：金融期权行情板，symbol 如华夏上证50ETF期权，end_month 如 2503。
- `option_current_em`
  用途：期权当日全市场行情。
- `option_sse_daily_sina`
  用途：上交所期权日线，symbol 为合约代码。
- `option_commodity_hist`
  用途：商品期权历史，exchange 为 dce/shfe/czce。
